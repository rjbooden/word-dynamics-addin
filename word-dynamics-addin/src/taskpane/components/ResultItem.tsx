...
export default class ResultItem extends React.Component<ResultItemProps> {
  ...
  onClick = async (notifyLoaded: () => void): Promise<void> => {
    await this.fillFields(this.props.entity, this.props.item).then(() => {
	...
	private getFieldValue(fieldName:string, item: any): string {
		if (fieldName.indexOf(',') < 0) {
			return item[fieldName] || " ";
		}
		else {
			const values: string[] = fieldName.split(',').map((name) => {
				return item[name]; 
			});
			let fieldValue: string = values.filter((entry: string) => /\S/.test(entry)).join(' ').trim();
  ...
  private async fillFields(entity: Entity, item: any, parentEntity: Entity = null): Promise<void> {
		...
		return new Promise<void>(async (resolve) => {
			for (let i = 0; i < entity.fields.length; i++) {
				...
				await Word.run(async (context) => {
					...
					let placeholder = doc.contentControls.getByTag(fieldName);
